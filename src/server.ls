...
app.all '*' (req, res, next) ->
	if (banned-ips.index-of req.ip) > -1
		...
		type = switch (req.hostname)
			| config.public-config.web-host => \web
			| config.public-config.api-host => \api
...
app.use vhost config.public-config.web-host, main-server
app.use vhost config.public-config.api-host, api-server
#app.use vhost \web-streaming.misskey.xyz web-streaming-server
#app.use vhost \streaming.misskey.xyz streaming-server
app.use vhost config.public-config.dev-host, dev-server
